...
export const GIT_POLICY_SUBAGENT = `Git: read-only only. Human commits; judge working tree. No reset/restore/commit/add/worktree.`;

/** Suggested bullet for spec ## Constraints when bootstrapping. */
export const GIT_POLICY_SPEC_CONSTRAINT =
  "Human commits only; agents use read-only git (status, diff, log) and must not reset, restore, commit, or use worktrees.";
